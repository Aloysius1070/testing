...
// Post file to backend and trigger download of processed _checked.xlsx (with async job polling)
export async function simulateGst(file, onProgress) {
  ...
  const uploadRes = await fetch(`${API_URL}/api/gst/check`, {
    method: 'POST',
    body: form,
    headers: headers,
    credentials: 'include'
  ...
  while (true) {
    ...
    const statusRes = await fetch(`${API_URL}/api/gst/status/${job_id}`, { credentials: 'include' });
    ...
    if (statusData.status === 'completed') {
      ...
      const downloadRes = await fetch(`${API_URL}/api/gst/download/${job_id}`, { credentials: 'include' });
